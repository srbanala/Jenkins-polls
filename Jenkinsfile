pipeline {
    agent any
     ...
     parameters{
         ...
         string(name: 'ec2-details',defaultValue: '10.0.1.66', description: 'test1 instance' )
         }
     stages {
       ...
       //TODO Uncommet the below block when using  multibranch pipeline
         ...
         */
         ...
         stage('Sonarqube_deploy') {
           steps{
                sh 'echo "Test instance ip address is ${ec2-details}" '
                sh 'ssh -o StrictHostKeyChecking=no ec2-user@"${ec2-details}" uptime'
                sh 'ssh -v ec2-user@"${ec2-details}" '
                sh 'scp -r docker-compose-install.sh ec2-user@"${ec2-details}":/tmp '
                sh 'ssh ec2-user@"${ec2-details}" sh /tmp/docker-compose-install.sh '
                sh 'scp -r docker-compose-sonarqube.yml ec2-user@"${ec2-details}":/home/ec2-user/docker-compose.yml '
                 //sh 'ssh ec-user@"${server2}" ./tmp/docker-compose-install.sh'
                sh 'ssh ec2-user@"${ec2-details}" docker-compose up -d '
         }
        }
